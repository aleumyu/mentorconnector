...
router.get("/api/v1/users/", function(req, res, next) {
  ...
  if (
    ...
    if (interestsParam) {
      interestsParam = interestsParam
        .split(",")
        .map(e => `i.interestTag="${e}"`)
        .join(" OR ");
      if (containOtherQuery) {
...
router.get("/api/v1/users/:id/favorites", function(req, res, next) {
  db(
    `SELECT f.userId, photo, industry, jobType, country, city, firstName, lastName, f.selectedUserId FROM user u INNER JOIN favorites f ON u.userId=f.selectedUserId WHERE f.userId=${
      req.params.id
    };`
  ).then(results => {
    if (results.error) {
...
router.post("/api/v1/users/:id/favorites", function(req, res, next) {
  db(
    `INSERT INTO favorites (userId, selectedUserId) VALUES (${req.params.id}, ${
...
router.delete("/api/v1/users/:id1/favorites/:id2", function(req, res, next) {
  db(
    `DELETE FROM favorites WHERE userId=${req.params.id1} AND selectedUserId=${
      req.params.id2
    };`
  ).then(results => {
    if (results.error) {
      ...
      bcrypt.hash(req.body.password, saltRounds, function(err, hash) {
        db(
          `INSERT INTO user (email, password) VALUES ("${
            req.body.email
          }", "${hash}"); SELECT LAST_INSERT_ID() as userId;`
        ).then(results => {
          if (results.error) {
            res.status(500).send(results.error);
          }
          const userId = results.data[0].insertId;
...
router.post("/api/v1/signin", function(req, res, next) {
  db(`SELECT email, password FROM user WHERE email="${req.body.email}"`).then(
    results => {
      if (results.data[0] && results.data[0].email !== req.body.email) {
        return res.status(409).send(results.error);
      } else if (results.data[0] && results.data[0].email === req.body.email) {
        bcrypt.compare(req.body.password, results.data[0].password, function(
          err,
          result
        ) {
          if (result == true) {
...
router.put("/api/v1/users/", function(req, res, next) {
  ...
  for (let i = 0; i < req.body.interestTag.length; i++) {
    if (i === req.body.interestTag.length - 1) {
      insertValues += `((SELECT userId FROM user WHERE email="${
        req.body.email
      }"), "${req.body.interestTag[i]}");`;
    } else {
      insertValues += `((SELECT userId FROM user WHERE email="${
        req.body.email
      }"), "${req.body.interestTag[i]}"), `;
  ...
  db(
    `UPDATE user SET photo="${req.body.photo}", industry="${
      req.body.industry
    }", jobType="${req.body.jobType}", years=${req.body.years}, intro="${
      req.body.intro
    }", country="${req.body.country}", city="${req.body.city}", role=${
      req.body.role
    }, meeting=${req.body.meeting}, firstName="${
      req.body.firstName
    }", lastName="${req.body.lastName}" WHERE email="${req.body.email}";`
  );
