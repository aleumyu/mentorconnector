...
router.get('/api/v1/users/:id', authenticationMiddleware(), (req, res, next) => {
	...
	db(`SELECT email FROM user WHERE email="${req.body.email}"`).then((results) => {
		if (results.data[0] && results.data[0].email === req.body.email) {
			...
			bcrypt.hash(req.body.password, saltRounds, function(err, hash) {
				db(
					`INSERT INTO user (email, password) VALUES ("${req.body
						.email}", "${hash}"); SELECT LAST_INSERT_ID() as userId;`
				).then((results) => {
					if (results.error) {
						res.status(500).send(results.error);
					}
...
router.post('/api/v1/signin', function(req, res, next) {
	db(`SELECT email, password FROM user WHERE email="${req.body.email}"`).then((results) => {
		if (results.data[0] && results.data[0].email !== req.body.email) {
			return res.status(409).send(results.error);
